...
3. **Reviewer dispatch** — determine which reviewers to invoke via the deterministic dispatcher script.
   - Run via Bash: `node scripts/reviewer-dispatch.mjs --handoff=<worktreePath>/docs/context/handoff.md --stage=implement --pipeline=refactor --run-id=<runId>`. Append `--force-review` if the operator's original `$ARGUMENTS` contains the literal token `[force-review]`.
