...
- **BLOCK** (any reviewer emitted BLOCK):
  ...
  Then write `<worktreePath>/.pipeline/gate-pending.json` with `{"runId":"<runId>","gate":"gate2","feature":"<feature name>","status":"pending","blockedBy":{"reviewer":"<reviewer name>","reason":"reviewer BLOCK in phase <N> — <reviewer>: <first line of violation>","phase":<N>}}`. Call `forge_update_run` with `status: "gate-pending"` and `gateState: {"gate":"gate2","status":"pending","feature":"<feature name>","createdAt":"<now ISO>","blockedBy":{"reviewer":"<reviewer name>","reason":"reviewer BLOCK in phase <N> — <reviewer>: <first line of violation>","phase":<N>}}`. Do NOT continue to the next phase, do NOT proceed to the completeness-checker. Exit the worker. The run stays gate-pending — observer surfaces the BLOCK via `blockedBy`, conductor can inline-fix the phase work and approve gate2 (continuing apply), or discard via `forge_update_run` `status: "discarded"`. The run is NOT marked failed.
...
5b. **Reviewer verdict handling** (only when step 5 ran):
   ...
   - If ANY reviewer emitted **REVISE** or yielded no-verdict (and none BLOCK):
     - If `N < 2`: increment `N` to `N+1`.
       ...
       3. Scan reviewer output files for `[needs-researcher]: <question>` signals. If any reviewer emitted this signal, the finding requires factual verification that coder revision alone cannot resolve (e.g. actual API contract, library behavior, module internals not visible in the diff). In that case: dispatch researcher before re-invoking the coder. The researcher output will be written to `docs/RESEARCH/` and available as context for the coder revision.
       4. Re-invoke the coder with `[revision-mode: N]` prepended to its prompt. If the failed-criteria list is non-empty, also prepend `[failed-criteria: <comma-joined AC-IDs>]` (e.g. `[failed-criteria: AC-2, AC-4]`). Pass all REVISE warnings as context. Then proceed to step 5c.
     - If `N >= 2`: call `forge_update_run` with `status: "failed"` and `failureReason: "REVISE unresolved after 2 revision passes — <comma-joined unresolved AC-IDs>"`. Do NOT write `gate-pending.json`. Do NOT open Gate #2. Log the unresolved AC-IDs and exit the worker. If any reviewer emitted `[needs-researcher]`, include the question in the failure reason so the conductor can decide whether to dispatch researcher manually before discarding.
