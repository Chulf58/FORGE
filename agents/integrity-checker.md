...
Run all ten checks below in order. For each check, emit a `[todo]` signal for every issue found using this exact format:

[todo] HIGH: <title> — <note>
[todo] MEDIUM: <title> — <note>
[todo] LOW: <title> — <note>

Severity mapping:
- `high` issues → `[todo] HIGH:`
- `medium` issues → `[todo] MEDIUM:`
- `low` issues → `[todo] LOW:`

If a check finds no issues, emit nothing for that check — no "all clear" signal per individual check.

After all ten checks, print one plain-text summary line:

Integrity check complete — <N> issues found

where N is the total count of `[todo]` signals emitted above (not counting this summary line). If N is 0, write "Integrity check complete — no issues found".
...
- If the file does not exist: emit `[todo] HIGH: board.json missing — pipeline task board cannot be read`
- If the file exists but is not valid JSON (you cannot parse it as a JSON object): emit `[todo] HIGH: board.json malformed JSON — pipeline task board cannot be read`
...
- If the file exists: read it. If the first heading is `# Handoff:`, emit: `[todo] MEDIUM: handoff.md exists — may be stale from an abandoned pipeline run; review or delete if no run is active`
...
- If the file does not exist: emit `[todo] HIGH: PLAN.md missing — no pipeline plan found`
...
- If unchecked-task count > 0 AND the `planned` array is empty or absent: emit `[todo] MEDIUM: PLAN.md has <N> unchecked tasks with no matching planned item on the board`
...
- If the file exists: count unchecked items (`- [ ]`).
  - If count > 0: emit `[todo] MEDIUM: TESTING.md has <N> unresolved test items`
...
[todo] LOW: custom agent shadows scaffold agent <basename> — review for intentional overrides
...
[todo] LOW: detected <indicator-pattern> files but stack not registered in project.json — consider adding via Project Overview
...
- If `docs/TESTING.md` exists and has more than 400 lines: check whether `docs/archive/` exists by globbing `docs/archive/*`.
  - If `docs/archive/` does not exist (glob returns no results and the directory itself is absent): emit `[todo] MEDIUM: docs/archive/ missing — TESTING.md exceeds 400 lines but documenter cannot archive old entries; create docs/archive/ to enable archival`
...
- If the file exists: parse it and read the `mode` and `startedAt` fields.
  ...
  - If `mode` starts with `apply` (apply feature, apply debug, apply refactor): emit `[todo] HIGH: stale run-active.json found — mode: <mode>, started: <startedAt as ISO string>. An interrupted apply pipeline may have left source files in a partially modified state.`
  - If `mode` does not start with `apply`: emit `[todo] MEDIUM: stale run-active.json found — mode: <mode>, started: <startedAt as ISO string>. A prior pipeline run may have been interrupted; delete this file if safe.`
...
`[todo] HIGH: required hook <name>.js missing — install from FORGE templates. Without it: <effect>`
...
- If a section's generated date is more than 90 days old, emit: `[todo] LOW: skills section "<stack name>" was generated <N> days ago — platform knowledge may be stale; re-run skills-generator or update manually`
...
- If the heading contains a `(generated: YYYY-MM-DD)` stamp and the date is more than 90 days old, emit: `[todo] LOW: capability skills file "<capability-id>" was generated <N> days ago — re-run skills-generator or update manually`
...
`[todo] HIGH: modules.json missing — run architect to generate`
...
`[todo] MEDIUM: module "<name>" references missing path: <path>`
...
`[todo] MEDIUM: module "<name>" references unknown module: <id>`

(d) **Missing description:** For any module with an empty description, emit:
`[todo] LOW: module "<name>" has no description`

---

## Output order

Emit all `[todo]` signals in check order (1 through 11), then the plain-text summary line last. Do not emit any other output after the summary line.
