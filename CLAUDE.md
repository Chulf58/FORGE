...
- **Conductor scratchpad (long inline edits):** For multi-step inline edits, write your approach to `.pipeline/conductor-scratchpad.md` before starting. Survives `/compact` and session resume — conversation history may not. Source: `docs/RESEARCH/agentic-context-content-management-for-token-reduction.md` §Compaction patterns.
- **Inline-work knowledge loop (MANDATORY — read before, write after):** Substantive inline work uses the knowledge base on both sides of the work — read prior patterns at the start, capture new learnings at the end. `resolveProjectDir()` (`mcp/lib/tools/shared.js:46`) resolves to the active project, so cross-project read/write works automatically (Dataverse work reads from and writes to Dataverse's `docs/solutions/`, plugin work reads from and writes to here).
  - **BEFORE substantive work — retrieve:** call `forge_get_patterns` with keywords from the user's request (feature/file/technology names, vendor names, error patterns). If matches return, read them — apply proven patterns and avoid rediscovering known gotchas. If `docs/gotchas/GENERAL.md` hasn't been read this session, Read it once — it's the project's procedural memory (always-applies rules, conventions).
  - **AFTER substantive work — capture:** call `forge_add_learning` before responding to the user. Lean toward capture — a redundant learning is easy to merge later via compound-refresh; a lost learning is unrecoverable.
  - **Substantive:** building a meaningful artifact (a Power Automate flow, an MCP tool, a hook script), debugging a non-obvious failure with a recoverable cause, working out a vendor/tool quirk that would take time to rediscover, finding a workaround for a missing vendor feature.
  - **NOT substantive:** status checks, file reads for navigation, TODO updates, single-line typo fixes, formatting changes, acknowledging runs, answering questions about existing code without changing it. Skip both retrieval and capture for these.
  - Pipeline runs already get retrieval (worker-task-inject + agent prompts call `forge_get_patterns`) and capture (documenter + auto-fire compound-refresh) — this rule covers the inline gap.
