 ...
 *
 * Strips characters that break shell quoting in either single- or double-quoted
 * arguments: `"`, `'`, `\`, backtick, `$`, carriage return, newline, and other
...
export function sanitizeFeatureName(raw) {
  ...
  return raw
    // Strip chars that break shell quoting (both ' and ") or trigger substitution
    .replace(/["'\\`$\r\n\x00-\x1f\x7f]/g, '')
    .trim()
    .slice(0, 200);
}
