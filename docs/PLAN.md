...
- [ ] 2. Fix forge_create_run plan-existence check (`mcp/server.js`)
  ...
  Verify: AC-2: When no gate1-approved plan run is found via `listRuns`, the fallback either checks each plan run's `worktreePath` for `docs/PLAN.md`, or skips the file-exists fallback entirely (relying solely on the gate1 approval status check); the main-root path `join(projectDir, 'docs', 'PLAN.md')` is not used as the sole fallback.
...
- [ ] 3. Clean stale PLAN.md from main project root (one-time cleanup task)
  ...
  Verify: AC-3: `C:\Users\cuj\forge-plugin\docs\PLAN.md` no longer contains any `### Feature:` content from old plan runs; either the file is deleted or truncated to a minimal stub `## Active Plan\n`; no other files in `docs/` (GENERAL.md, ARCHITECTURE.md, DECISIONS.md, etc.) are removed.
...
- [ ] 4. Add worktree-boundary guardrail to planner agent instructions (`agents/planner.md`)
  ...
  Verify: AC-4: `agents/planner.md` contains an explicit statement that `docs/PLAN.md` must be written relative to the worker's cwd (the worktree), and a note that the absolute path must contain `.worktrees/` when running in a worktree context; the planner's `### Always` section references this constraint.
...
- [ ] 5. Add PreToolUse guard script that blocks PLAN.md writes to main project root (`hooks/workflow-guard.js`)
  ...
  Verify: AC-5: When a Write or Edit tool call targets a path whose resolved absolute form ends with `docs/PLAN.md` and the worker's cwd contains `.worktrees/` but the target path does NOT start with the worker's cwd, `workflow-guard.js` blocks the call (exit 2) with a descriptive error message; writes to `<worktreePath>/docs/PLAN.md` pass through; writes from the main root (no `.worktrees/` in cwd) pass through unchanged.
