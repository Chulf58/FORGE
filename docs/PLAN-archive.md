...
### [x] Feature: Fix stale run-active.json pointer pollution
...
### [x] Feature: Hello World Slash Command

- [x] 1. Create the `/forge:hello` slash command file (`commands/forge/hello.md`)
...
### [x] Feature: Add gitIntegration to ALLOWED_CONFIG_KEYS

- [x] 1. Add `"gitIntegration"` to the `ALLOWED_CONFIG_KEYS` array in `mcp/server.js` (`mcp/server.js`)
  Append `"gitIntegration"` to the `ALLOWED_CONFIG_KEYS` array. The value is an object — the allowlist check gates on the key name only; no type validation of the object is required at this layer.
  Verify: `ALLOWED_CONFIG_KEYS` in `mcp/server.js` includes `"gitIntegration"`; `forge_update_config` accepts `{ gitIntegration: { enabled: false } }` without returning an "Unknown config key" error.
