## [2026-04-20] Fix first-boot MCP — seed launcher + committed node_modules

- Committed `bin/forge-mcp-server.cmd` seed with node discovery (PATH → Program Files → nvm-windows)
- Cleaned `mcp/package.json` — removed TUI deps (blessed, ink, react, node-pty, xterm, pngjs) that don't belong in the MCP server
- Committed pruned `mcp/node_modules/` (~6.6 MB, runtime-only files) — MCP server works offline on first boot, no npm install needed
- Added `.gitignore` exception for `mcp/node_modules/`
- Added board tasks: `/forge:doctor` (`b335d4d1`), MCP-missing messaging (`094aaab3`)

---

## [2026-04-20] Fix first-boot MCP server — commit seed forge-mcp-server.cmd

- Committed `bin/forge-mcp-server.cmd` as a seed with node discovery (PATH → Program Files → nvm-windows). Previously gitignored and only generated by SessionStart hook, creating a chicken-and-egg: MCP server tried to start before the hook created the .cmd file.
- Removed `bin/forge-mcp-server.cmd` from `.gitignore` with explanatory comment
- SessionStart hook still overwrites with absolute node path for faster startup
- Added board tasks: `/forge:doctor` diagnostic command (`b335d4d1`), MCP-missing user messaging (`094aaab3`)
