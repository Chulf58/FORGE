...
> **Looking for technical reference?** Agent tables (25 active), signal protocol, pipeline modes, hook system (29 registered scripts across 10 event types), MCP tools (40), model routing, and key files live in [FORGE-REFERENCE.md](FORGE-REFERENCE.md) — generated on demand from source-of-truth files.
...
1. Providing 25 specialist agents loaded via the plugin system
2. Exposing 33 skills (slash commands) that orchestrate those agents into pipelines
3. Enforcing workflow rules through 29 registered hook scripts across 10 lifecycle events
4. Offering 40 MCP tools for structured access to pipeline state
...
|---|---|---|
| Named, pre-defined pipelines | Yes — 8 pipeline types, 3 modes | No — manual sequencing required |
| Human approval gates | Yes — Gate #1 (plan→implement), Gate #2 (review→apply), commit gate | No |
| 25 specialist agents | Yes — plan, implement, review, debug, refactor, architect, adversarial, knowledge | Possible via `--agents` but no orchestration |
| Parallel reviewer waves | Yes — up to 4 reviewers simultaneously | No built-in review step |
...
| Enforcement hooks | Yes — bash-guard, workflow-guard, role-based access, stuck-loop guard | No |
| 40 MCP tools for pipeline state | Yes — board, config, gates, modules, model routing, dashboard state, knowledge store, escalation | No |
| Multi-model routing | Yes — per-agent model selection, external provider support (OpenAI, Gemini) | Single model per session |
...
| Worker sessions | Yes — autonomous pipeline workers with conductor/worker session model | No |
...
| Pipeline modes | 3 modes (supervised/autonomous/direct) controlling reviewer depth | Single mode |
| Persistent project state | Yes — board, modules, run lifecycle, usage tracking | Stateless per run |
| MCP tool layer | Yes — 40 structured tools for pipeline state | No |
...
| Enforcement hooks | Yes — 4 PreToolUse hooks with exit 2 blocking + TDD guard | No enforcement hooks |
| Pipeline modes | 3 modes controlling agent depth (supervised/autonomous/direct) | Single mode |
| Multi-model routing | Yes — MCP-based, per-agent, external provider support | No |
| MCP tool layer | Yes — 40 tools for structured state access | No |
...
- Classifier gap fixes: SUPERVISED mode distinct UX (amber chip, "direct edit — no agents will run"), stale chip clear on textarea edit, Escape key dismissal, apply modes excluded from classification
...
*Source: `.pipeline/board.json` open items as of 2026-05-28. 24 open items across 17 tags.*

**Deterministic pre-review (highest cluster):** AST-aware risk classifier (tree-sitter based, replaces keyword scan in `lean-risk-classify.mjs`), post-reviewer verdict aggregation into structured digest, phase-verifier agent to verify each phase's Verify: lines pass before next phase. These items form the `epic:deterministic-pre-review` cluster.

**Thin orchestrator (high priority):** The deterministic orchestrator opt-in flags (`FORGE_ORCHESTRATOR_PLAN`, `FORGE_ORCHESTRATOR_IMPLEMENT`) are built but the full "thin skill prose → state machine" transition is tracked as `epic:thin-orchestrator` (TODO 286e2c93). Goal: eliminate LLM prose-following from the conductor loop entirely. An `apply-worker` race condition (commit gate data-loss risk during concurrent run.json writes) is the most urgent blocker in this cluster.

**User-first planning (high priority):** Full user-first planning flow — tight grill-me interview, skill vendoring, and knowledge-loop closure before planner dispatch (TODO 828be04f, `epic:user-first-planning`). Phase progress UI in the TUI observer (TODO e7ebd631).

**Evaluation and quality:** Evaluation suite to replay pipeline runs and measure agent quality (TODO 65c1ad5f). LLM-judge for the eval suite (deferred — Playwright/non-Anthropic model dependency, TODO 1037a796). Gate-verification digest (non-blocking spot-check skill, TODO f02a9d9a).

**Worktree lifecycle:** Cluster of worktree-related gaps tracked as `epic:worktree-lifecycle` — including investigating the root cause of `git merge-cascade` node_modules deletion (TODO 83079dea).

**Skill quality and observability:** Skill-feedback observation lane (TODO 8c7393fc) and `/forge:approve` gate-approval-token fix on gate1 (TODO 250553e5). In-session subagent dispatches not accounted for in run lifecycle (TODO afaa578d).

**MCP and tooling gaps:** `forge_update_task` lacks a `tags` field (TODO 3c080ddf). `aborted_streaming` surfacing in worker harness (TODO bd5a39d3). TDD guard runNodeTest timeout fix (TODO 859d5830).

**External model routing:** `forge_call_external`, router, config, and usage tracking are built. OpenAI and Gemini adapters work. External supervisor transport via Playwright/browser automation is parked (TODO fcf9bc3f).
...
**Skills replaced commands.** The 17 `commands/forge/*.md` files were migrated to `skills/<name>/SKILL.md` format in v0.2.0+. The plugin now has 33 skills. Skills use `context: fork` for significant token savings — the skill prompt runs in a forked context rather than loading the full conversation. The old `commands/forge/` directory was removed (two legacy commands `hello.md` and `doctor.md` remain in `commands/forge/`). All skill `name:` fields carry the `forge:` prefix to prevent command-shadowing collisions with Claude Code's native commands (a bare `name: config` once silently shadowed Claude Code's native `/config` command — fixed with blanket prefix policy).

---

*Last updated: 2026-05-28. For complete technical reference, see [FORGE-REFERENCE.md](FORGE-REFERENCE.md). For architecture decisions, see `docs/DECISIONS.md`. For the update recipe, see `docs/FORGE-OVERVIEW-RECIPE.md`.*
