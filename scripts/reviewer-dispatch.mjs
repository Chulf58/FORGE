...
if (isMainModule()) {
  ...
  if (reviewerOverrides !== null) {
    // At plan stage, technical-skeptic is non-negotiable — always include it regardless of
    // reviewerOverrides. The override mechanism locks in the conductor-approved reviewer
    // team, but technical-skeptic's cross-model critique is a pipeline invariant at plan stage
    ...
    // technical-skeptic was added to dispatchForPlanStage — so it was never in the approved list.
    const effectiveOverrides = (stage === 'plan' && !reviewerOverrides.includes('technical-skeptic'))
      ? [...reviewerOverrides, 'technical-skeptic'].sort()
      : reviewerOverrides;
