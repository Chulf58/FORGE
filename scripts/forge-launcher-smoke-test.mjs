...
async function main() {
  ...
  // 4. Windows shim — generated by hooks/mcp-deps-install.js on SessionStart with
  // absolute per-user node paths. Not tracked in git (gitignored). Skip if absent;
  // verify content only when present (e.g. after a local SessionStart has run).
  const shimPresent = existsSync(SHIM);
  if (shimPresent) {
    const shimSrc = readFileSync(SHIM, 'utf-8');
    if (!/^@echo off/im.test(shimSrc)) {
      fail('bin/forge.cmd does not start with `@echo off`');
    }
    if (!/forge\.js/.test(shimSrc)) {
      fail('bin/forge.cmd does not reference forge.js — got: ' + JSON.stringify(shimSrc));
    }
  }
  ...
  console.log('  bin/forge.cmd Windows shim ' + (shimPresent ? 'invokes node against forge.js' : 'not present (gitignored — generated at SessionStart)'));
